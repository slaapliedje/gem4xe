...
| `make test-m37` | PASS | **a cartridge** — `tools/mkcar.py` builds a 1 MB AtariMax image and the machine boots it with no disk and no DOS at all, which is what makes a one-file demo possible: the release's own `.atr` answers BOOT ERROR on its own, wanting SpartaDOS X in the machine. The header, the type and the checksum are checked on the host and the boot on the machine, because a wrong type or a bootstrap in the wrong bank gives a cartridge that is perfectly valid and does **nothing** — silence, not an error. Bank 127 is the one an AtariMax maps at reset, so that is where the bootstrap goes; moving it to bank 0 reddens this gate, which is how it was proven to speak. Step one of three (`docs/cartridge.md`): the CPU switch and the staging go on top, one at a time |
